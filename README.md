...
Directly answerable: Q-002 (Viewer + API credentials, single doc)
Requires two documents: Q-001 (timezone change + missed export, spans KB-003 + KB-004)
Ambiguous, requires clarification: Q-003 ("sync is not working")
Out-of-scope: Q-005 (refund request + prompt-injection attempt)
Initial answer fails verification: run any question with `--demo-verification-failure` (see above)
